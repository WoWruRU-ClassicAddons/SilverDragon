		...
		args={
			settings = {
				...
				args={
					...
					sound = {
						name=L["Sound"], desc=L["Play sound when an RARE is found"],
						type="toggle",
			...
			if not cartdb_populated then
				for zone, mobs in pairs(self.db.profile.mobs) do
					for name in pairs(mobs) do
						...
						if x > 0 and y > 0 then
							if elite == 1 then
								Cartographer_Notes:SetNote(zone, tonumber(x)/100, tonumber(y)/100, 'Rare', 'SilverDragon', 'title', name .. L[" - |cFF00FF00Lv"] .. level .. L[" |cFFFF0000Elite"])
							else
								Cartographer_Notes:SetNote(zone, tonumber(x)/100, tonumber(y)/100, 'Rare', 'SilverDragon', 'title', name .. L[" - |cFF00FF00Lv"] .. level)
							end
						end
					end
				end
...
function SilverDragon:Announce(name, dead)
	...
	if (not self.lastseen[name]) or (self.lastseen[name] < (time() - 600)) then
		...
		if self.db.profile.announce.sound then
			TargetByName( name )
			PlaySoundFile("Interface\\AddOns\\SilverDragon\\Sound\\event_wardrum_ogre.wav" )
			PlaySoundFile("Interface\\AddOns\\SilverDragon\\Sound\\scourge_horn.wav")
		end
